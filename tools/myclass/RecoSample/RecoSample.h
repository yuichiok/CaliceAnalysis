...
// Sat Sep 17 23:50:24 2022 by ROOT version 6.26/06
// from TTree ecal/Build ecal events
// found on file: ../../data/reco/raw_siwecal_90320_e-_10.0GeV_build.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           hit_slab[1240];   //[nhit_len]
   Int_t           hit_chip[1240];   //[nhit_len]
   Int_t           hit_chan[1240];   //[nhit_len]
   Int_t           hit_sca[1240];   //[nhit_len]
   Float_t         hit_x[1240];   //[nhit_len]
   Float_t         hit_y[1240];   //[nhit_len]
   Float_t         hit_z[1240];   //[nhit_len]
   Int_t           hit_adc_high[1240];   //[nhit_len]
   Int_t           hit_adc_low[1240];   //[nhit_len]
   Float_t         hit_energy[1240];   //[nhit_len]
   Float_t         hit_energy_lg[1240];   //[nhit_len]
   Int_t           hit_n_scas_filled[1240];   //[nhit_len]
   Int_t           hit_isHit[1240];   //[nhit_len]
   Int_t           hit_isMasked[1240];   //[nhit_len]
   Int_t           hit_isCommissioned[1240];   //[nhit_len]
...
#ifdef RecoSample_cxx
RecoSample::RecoSample(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../../data/reco/raw_siwecal_90320_e-_10.0GeV_build.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../../data/reco/raw_siwecal_90320_e-_10.0GeV_build.root");
      }
...
RecoSample::~RecoSample()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t RecoSample::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t RecoSample::LoadTree(Long64_t entry)
{
...
void RecoSample::Init(TTree *tree)
{
...
void RecoSample::Show(Long64_t entry)
{
...
Int_t RecoSample::Cut(Long64_t entry)
{
